...
void UGTSceneCaptureComponent2D::CaptureImage(FGTImage& OutImage)
{
    ...
    ReadSurfaceDataFlags.SetLinearToGamma(bUseDisplayGamma);
...
void UGTSceneCaptureComponent2D::StartCaptureImageAsync(
    int32 FrameId,
    ...
    FSessionValidationFunc SessionValidator)
{
    ...
    {
        UE_LOG(LogTemp, Warning, TEXT("UnrealGT async capture failed: RenderTargetResource is null for frame %d."), FrameId);
        return;
    }

    // Create new readback for this capture
    FPendingReadback PendingReadback;
    PendingReadback.Readback = MakeUnique<FRHIGPUTextureReadback>(TEXT("GTSceneCaptureGPUReadback"));
    PendingReadback.FrameId = FrameId;
    PendingReadback.TimeStamp = TimeStamp;
    PendingReadback.SessionId = SessionId;
    PendingReadback.SessionValidator = MoveTemp(SessionValidator);

    // Enqueue GPU readback on render thread
    ENQUEUE_RENDER_COMMAND(EnqueueGTAsyncReadback)(
        [Readback = PendingReadback.Readback.Get(), RTRes = RenderTargetResource](FRHICommandListImmediate& RHICmdList) {
    ...
    // Add to queue
    PendingReadbacks.Add(MoveTemp(PendingReadback));
}

bool UGTSceneCaptureComponent2D::IsAsyncCaptureReady() const
{
    return PendingReadbacks.Num() > 0 && 
           PendingReadbacks[0].Readback.IsValid() && 
           PendingReadbacks[0].Readback->IsReady();
}

bool UGTSceneCaptureComponent2D::FinishCaptureImageAsync(
    FGTImage& OutImage,
    int32& OutFrameId,
    ...
    FSessionValidationFunc& OutSessionValidator)
{
    ...
    FPendingReadback& FrontReadback = PendingReadbacks[0];
    if (!FrontReadback.Readback.IsValid() || !FrontReadback.Readback->IsReady())
    {
        return false;
    }
    ...
    OutImage.SetSize(Width, Height);
    OutFrameId = FrontReadback.FrameId;
    OutTimeStamp = FrontReadback.TimeStamp;
    OutSessionId = FrontReadback.SessionId;
    OutSessionValidator = MoveTemp(FrontReadback.SessionValidator);

    int32 RowPitchInPixels = 0;
    uint8* Ptr = (uint8*)FrontReadback.Readback->Lock(RowPitchInPixels);
    if (!Ptr || RowPitchInPixels < Width)
    {
        if (Ptr) FrontReadback.Readback->Unlock();

        UE_LOG(LogTemp, Warning, TEXT("UnrealGT async GPU readback failed for frame %d. This frame will be skipped."), FrontReadback.FrameId);

        PendingReadbacks.RemoveAt(0);
        return false;
    }

    // Handle float vs color readback similar to sync version
    if (bColorAsFloat)
    {
        FrontReadback.Readback->Unlock();

        UE_LOG(LogTemp, Warning, TEXT("UnrealGT async GPU readback does not support bColorAsFloat yet. Frame %d will be skipped."), FrontReadback.FrameId);

        PendingReadbacks.RemoveAt(0);
        return false;
    }

    // Copy pixel data from readback to image
    const int32 BytesPerPixel = 4;
    const int32 SrcRowBytes = RowPitchInPixels * BytesPerPixel;
    const int32 DstRowBytes = Width * BytesPerPixel;
    
    OutImage.Pixels.SetNumUninitialized(Width * Height);
    uint8* Dst = (uint8*)OutImage.Pixels.GetData();
    
    for (int32 y = 0; y < Height; ++y)
    {
        FMemory::Memcpy(
            Dst + (size_t)y * DstRowBytes, 
            Ptr + (size_t)y * SrcRowBytes, 
            DstRowBytes);
    }
