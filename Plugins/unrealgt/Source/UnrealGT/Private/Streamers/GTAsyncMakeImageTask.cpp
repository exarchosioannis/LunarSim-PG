...
FGTAsyncMakeImageTask::FGTAsyncMakeImageTask(
    ...
    , SessionId(SessionId)
    , SessionValidator(MoveTemp(SessionValidator))
...
{
    // Validate session before any processing - discard stale tasks
    ...
    const FGTImage& WorkingImage = Image;
    ...
    switch (ImageFormat)
    {
        case EGTImageFileFormat::PNG:
        {
            ...
            // Copy pixels and ensure opaque alpha for PNG export
            TArray<FColor> PngPixels = WorkingImage.Pixels;
            for (FColor& Pixel : PngPixels)
            {
                Pixel.A = 255;
            }
            
            const int32 BufferSize = PngPixels.Num() * sizeof(FColor);
        ...
        case EGTImageFileFormat::BMP:
        {
            ...
            // Image.Width, Image.Height, ERGBFormat::RGBA, 8); ImgData =
            ...
            const FColor* Data = WorkingImage.Pixels.GetData();
...
#pragma pack(pop)
    ...
    int32 FrameIndexLocal = FrameIndex;
    ...
    // gamethread
    AsyncTask(ENamedThreads::GameThread, [SourceComponentLocal, ImgData, TimeStamptLocal, FrameIndexLocal]() {
        if (SourceComponentLocal && SourceComponentLocal->IsValidLowLevelFast())
        {
            SourceComponentLocal->DataReadyDelegate.Broadcast(ImgData, TimeStamptLocal, FrameIndexLocal);
        }
    });
}
